...
# class Implicit2StageIter:
#     """
#     Base class for implicit iterative methods.
#     """

#     @staticmethod
#     def calc_jac_itm(tds, gxs, gys):
#         pass

#     @staticmethod
#     def calc_jac_bdf(tds,gxs,gys):
#         pass

#     @staticmethod
#     def calc_q_itm(x, f, Tf, h, x0, f0):
#         pass

#     @staticmethod
#     def clac_q_bdf(x0_itm,x, f, Tf, h, x0, f0):
#         pass
...
#         x0_prior = np.copy(tds.x0)
#         y0_prior = np.copy(tds.y0)
#         f0_prior = np.copy(tds.f0)
...
#             # solve implicit trapezoidal method (ITM) integration
#             if tds.config.g_scale > 0:
#                 gxs = tds.config.g_scale * gamma*tds.h * dae.gx
#                 gys = tds.config.g_scale * gamma*tds.h * dae.gy
#             else:
#                 gxs = dae.gx
#                 gys = dae.gy

#             # calculate complete Jacobian matrix ``Ac```
#             tds.Ac = tds.method.calc_jac_itm(tds, gxs, gys)

#             # equation `tds.qg[:dae.n] = 0` is the implicit form of differential equations using ITM
#             tds.qg[:dae.n] = tds.method.calc_q_itm(dae.x, dae.f, dae.Tf, tds.h, tds.x0, tds.f0)
...
#             # debug outputs
#             if system.options.get("verbose", 20) <= 10:
#                 tqdm.write(f'* Max. iter. {tds.config.max_iter} reached for ITM t={dae.t:.6f}s, '
#                            f'h={tds.h:.6f}s, max inc={mis:.4g} ')
...
#             if system.options.get("verbose", 20) <= 10:
#                 tqdm.write(f'ITM Converged in {tds.niter} steps for t={dae.t:.6f}s, '
#                            f'h={tds.h:.6f}s, max inc={mis:.4g} ')
...
#         x0_itm = np.copy(dae.x[:])
#         y0_itm = np.copy(dae.y[:])
...
#         # x0_prior = np.copy(dae.x)
#         # y0_prior = np.copy(dae.y)
#         # f0_prior = np.copy(dae.f)
#         # tds.f0_itm[:] = dae.f
...
#             # calculate complete Jacobian matrix ``Ac```
#             tds.Ac = tds.method.calc_jac_bdf(tds, gxs, gys)

#             # equation `tds.qg[:dae.n] = 0` is the implicit form of differential equations using ITM
#             tds.qg[:dae.n] = tds.method.calc_q_bdf(x0_itm,dae.x, dae.f, dae.Tf, tds.h, tds.x0, tds.f0)
...
#             # debug outputs
#             if system.options.get("verbose", 20) <= 10:
#                 tqdm.write(f'* Max. iter. {tds.config.max_iter} reached for BDF t={dae.t:.6f}s, '
#                            f'h={tds.h:.6f}s, max inc={mis:.4g} ')
...
#             if system.options.get("verbose", 20) <= 10:
#                 tqdm.write(f'BDF Converged in {tds.niter} steps for t={dae.t:.6f}s, '
#                            f'h={tds.h:.6f}s, max inc={mis:.4g} ')
...
# class TRBDF2(Implicit2StageIter):
#     """
#     Trapezoidal methods.
#     """
...
#         return sparse([[tds.Teye - gamma * tds.h * 0.5 * dae.fx, gxs],
#                        [-gamma * tds.h * 0.5 * dae.fy, gys]], 'd')
    
#     @staticmethod
#     def calc_jac_bdf(tds, gxs, gys):
#         """
#         Build full Jacobian matrix ``Ac`` for BDF method.
#         """
...
#         return sparse([[tds.Teye - ((1 - gamma)/(2 - gamma)) * tds.h * dae.fx, gxs],
#                        [- ((1 - gamma)/(2 - gamma)) * tds.h * dae.fy, gys]], 'd')
...
#         return Tf * (x - x0) - gamma * h * 0.5 * (f + f0)
#         # return 1 * (x - x0) - gamma * h * 0.5 * (f + f0)
    
#     @staticmethod
#     def calc_q_bdf(x0_itm, x, f, Tf, h, x0, f0):
#         """
#         Calculate the residual of algebraized differential equations.
...
#         return Tf * (x - (1/(gamma*(2-gamma)))*x0_itm + ((1-gamma)**2/(gamma*(2-gamma)))*x0) - ((1-gamma)/(2-gamma))*h*f0
#         # return Tf * x - (1/(gamma*(2-gamma)))*x0_itm + ((1-gamma)**2/(gamma*(2-gamma)))*x0 - ((1-gamma)/(2-gamma))*h*f0
#         # return x - ((1-gamma)/(2-gamma))*h*f0 - (1/(gamma*(2-gamma)))*x0_itm + ((1-gamma)**2/(gamma*(2-gamma)))*x0
