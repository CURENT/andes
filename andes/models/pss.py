...
class PSS1(ModelBase):
    ...
    def servcall(self, dae):
        self.get_field_ext('AVR', 'syn', 'syn', self.avr)
        self.get_field_ext('AVR', 'u', 'uavr', self.avr)
        self.get_field_ext('Synchronous', 'u', 'usyn', self.syn)
        self.get_field_ext('Synchronous', 'bus', 'bus', self.syn)
        self.get_field_ext('Synchronous', 'Sn', 'Sg', self.syn)
        self.get_field_ext('Synchronous', 'v', 'v', self.syn)
        self.get_field_ext('Synchronous', 'vf', 'vf', self.syn)
        self.get_field_ext('Synchronous', 'pm', 'pm', self.syn)
        self.get_field_ext('Synchronous', 'omega', 'omega', self.syn)
        self.get_field_ext('Synchronous', 'p', 'p', self.syn)
        self.get_field_ext('BusFreq', 'w', 'w', self.bus)
    ...
    def gcall(self, dae):
        dae.g[self.In1] = mul(self.u0, -dae.y[self.In1] + mul(self.Ic11, -1 + dae.x[self.omega]) + mul(self.Ic12, -1 + dae.x[self.w]) + mul(self.Ic15, dae.y[self.v]) + mul(self.Ic13, dae.y[self.p], self.toSg) + mul(self.Ic14, dae.y[self.pm], self.toSg))
        dae.g[self.In2] = mul(self.u0, -dae.y[self.In2] + mul(self.Ic21, -1 + dae.x[self.omega]) + mul(self.Ic22, -1 + dae.x[self.w]) + mul(self.Ic25, dae.y[self.v]) + mul(self.Ic23, dae.y[self.p], self.toSg) + mul(self.Ic24, dae.y[self.pm], self.toSg))
        dae.g[self.In] = mul(self.u0, dae.y[self.In1] + dae.y[self.In2] - dae.y[self.In])
        dae.g[self.x3] = mul(self.u0, -dae.x[self.u3] - dae.y[self.x3] + mul(dae.y[self.In], self.T34))
        dae.g[self.x4] = mul(self.u0, dae.x[self.u4] - dae.y[self.x4] + mul(self.T56, dae.y[self.x3]))
        dae.g[self.x5] = mul(self.u0, dae.x[self.u5] - dae.y[self.x5] + mul(self.T78, dae.y[self.x4]))
        dae.g[self.x6] = mul(self.u0, dae.x[self.u6] - dae.y[self.x6] + mul(self.T910, dae.y[self.x5]))
        ...
        dae.g += spmatrix(mul(self.u0, dae.y[self.vst]), self.vf, [0] * self.n, (dae.m, 1), 'd')
    ...
    def jac0(self, dae):
        ...
        dae.add_jac(Fy0, mul(self.K1, self.u0, div(1, self.T1)), self.x1, self.In1)
        dae.add_jac(Fy0, mul(self.K2, self.u0, div(1, self.T2)), self.x2, self.In2)
        dae.add_jac(Fy0, mul(self.T34, self.u0, div(1, self.T4)), self.u3, self.In)
        dae.add_jac(Fy0, mul(self.u0, div(1, self.T6), 1 - self.T56), self.u4, self.x3)
        dae.add_jac(Fy0, mul(self.u0, div(1, self.T8), 1 - self.T78), self.u5, self.x4)
        dae.add_jac(Fy0, mul(self.u0, div(1, self.T10), 1 - self.T910), self.u6, self.x5)
...
class PSS2(ModelBase):
    ...
    def gcall(self, dae):
        dae.g[self.In] = mul(self.u0, -dae.y[self.In] + mul(self.Ic1, -1 + dae.x[self.omega]) + mul(self.Ic2, -1 + dae.x[self.w]) + mul(self.Ic5, dae.y[self.v]) + mul(self.Ic3, dae.y[self.p], self.toSg) + mul(self.Ic4, dae.y[self.pm], self.toSg))
        dae.g[self.v1] = mul(self.u0, dae.x[self.q0] - dae.y[self.v1] + mul(self.A5, dae.x[self.q1]) + mul(self.A6, dae.x[self.q2]))
        dae.g[self.v2] = mul(self.u0, dae.x[self.x1] - dae.y[self.v2] + mul(self.T12, dae.y[self.v1]))
        dae.g[self.v3] = mul(self.u0, dae.x[self.x2] - dae.y[self.v3] + mul(self.T34, dae.y[self.v2]))
        dae.g[self.v4] = mul(self.u0, -dae.y[self.v4] - dae.x[self.x3] + mul(self.KsT56, dae.y[self.v3]))
    ...
    def fcall(self, dae):
        ...
        dae.f[self.q3] = mul(self.u0, div(1, self.H4), dae.y[self.In] - dae.x[self.q0] - mul(self.H1, dae.x[self.q1]) - mul(self.H2, dae.x[self.q2]) - mul(self.H3, dae.x[self.q3]))
        dae.f[self.x1] = mul(self.u0, div(1, self.T2), -dae.x[self.x1] + mul(dae.y[self.v1], 1 - self.T12))
        dae.f[self.x2] = mul(self.u0, div(1, self.T4), -dae.x[self.x2] + mul(dae.y[self.v2], 1 - self.T34))
        dae.f[self.x3] = mul(self.u0, div(1, self.T6), -dae.x[self.x3] + mul(self.KsT56, dae.y[self.v3]))
    ...
    def jac0(self, dae):
        ...
        dae.add_jac(Fx0, - mul(self.H3, self.u0, div(1, self.H4)), self.q3, self.q3)
        dae.add_jac(Fx0, - mul(self.u0, div(1, self.H4)), self.q3, self.q0)
        dae.add_jac(Fx0, - mul(self.H2, self.u0, div(1, self.H4)), self.q3, self.q2)
        dae.add_jac(Fx0, - mul(self.H1, self.u0, div(1, self.H4)), self.q3, self.q1)
        ...
        dae.add_jac(Fy0, mul(self.u0, div(1, self.T2), 1 - self.T12), self.x1, self.v1)
        dae.add_jac(Fy0, mul(self.u0, div(1, self.T4), 1 - self.T34), self.x2, self.v2)
        dae.add_jac(Fy0, mul(self.KsT56, self.u0, div(1, self.T6)), self.x3, self.v3)
        dae.add_jac(Gy0, 1e-6, self.In, self.In)
