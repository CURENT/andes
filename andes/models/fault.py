    ...
    def checktime(self, actual_time):
        ...
        for i in range(self.n):
            if self.tf[i] == self.time:
                self.system.Log.info('Applying fault on {} at {}.'.format(self.bus[i], self.tf[i]))
                self.u[i] = 1
                self.active += 1
                self.angle0 = self.system.DAE.y[self.system.Bus.a]
                self.volt0 = self.system.DAE.y[self.system.Bus.n:2*self.system.Bus.n]
                self.system.DAE.factorize = True

            elif self.tc[i] == self.time:
                self.system.Log.info('Clearing fault on {} at {}.'.format(self.bus[i], self.tc[i]))
                self.u[i] = 0
                self.active -= 1
                self.system.DAE.y[self.system.Bus.n:2*self.system.Bus.n] = self.volt0
