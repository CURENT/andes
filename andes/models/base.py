...
class Model(object):
        ...
        for it, dest in zip(iter_list, dest_list):
            for name, instance in it.items():
                if instance.equation is None:
                    ...
                    dest.append(sympify(instance.equation, locals=self.input_syms))
    ...
    def gcall(self):
        ...
        # add values to corresponding equations
        idx = 0
        # TODO: cache-able below
        algeb_dict = OrderedDict(list(self.algebs.items()) +
                                 list(self.vars_ext.items()))

        for name, instance in algeb_dict.items():
