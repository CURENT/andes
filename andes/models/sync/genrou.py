...
class GENROUOSData(GENBaseOSData):
    """
    GENROU with operator splitting data.
    """
...
class GENROUOSModel:
    def __init__(self):
        ...
        self.XadIfd.v_str = 'u * (e1q + (xd-xd1) * (gd1*Id - gd2*e2d + gd2*e1q) + Se*psi2d)'
...
class GENROUOS(GENROUOSData, GENBaseOS, GENROUOSModel, Flux0):
    """
    Round rotor generator with quadratic saturation.
    """

    def __init__(self, system, config):
        GENROUOSData.__init__(self)
        GENBaseOS.__init__(self, system, config)
        Flux0.__init__(self)
