...
class WTG3(ModelBase):
    ...
    def __init__(self, system, name):
        ...
        self._service.extend(['pi', 'u0', 'phi', 'vref0', 'x0'])
        self._states.extend(['theta_p', 'omega_m', 'ird', 'irq'])
        self._times.extend(['Tp', 'Te'])
        self._data.update({'alf_s': 0, 'u': 1, 'pmax': 1, 'gen': 0, 'R': 1, 'Sn': 100, 'pmin': 0, 'Tp': 0.3, 'H': 2, 'xr': 1, 'qmax': 1, 'qmin': 0, 'ngb': 1, 'Te': 0.01, 'rs': 0, 'rr': 0, 'xmu': 1, 'Ks': 1, 'nblade': 3, 'npole': 4, 'Kp': 1, 'ngen': 1, 'KV': 1, 'gammaq': 1, 'wind': 0, 'rho': 1, 'gammap': 1, 'xs': 1})
        self._descr.update({'alf_s': 'Shadow effect factor', 'pmax': 'Maximum active power', 'gen': 'Static generator idx', 'R': 'Rotor radius', 'pmin': 'Minimum reactive power', 'Tp': 'Pitch control time constant', 'H': 'Machine rotor and turbine inertia constant', 'xr': 'Rotor reactance', 'qmax': 'Maximum active power', 'qmin': 'Minimum reactive power', 'ngb': 'Gear box ratio', 'Te': 'Power control time constant', 'rs': 'Stator resistance', 'rr': 'Rotor resistance', 'xmu': 'Magnetizing reactance', 'Ks': 'Shaft stiffness', 'nblade': 'Number of blades', 'npole': 'Number of poles', 'Kp': 'Pitch control gain', 'ngen': 'Number of generators', 'KV': 'Voltage control gain', 'gammaq': 'Reactive power generation ratio', 'wind': 'Wind time series idx', 'rho': 'Air density', 'gammap': 'Active power generation ratio', 'xs': 'Stator reactance'})
        self._units.update({'rho': 'kg/m^3', 'rr': 'pu', 'xmu': 'pu', 'rs': 'pu', 'Ks': 'pu', 'pmax': 'pu', 'Kp': 'pu', 'Tp': 's', 'KV': 'pu', 'pmin': 'pu', 'R': 'm', 'H': 'MWs/MVA', 'Te': 's', 'qmax': 'pu', 'qmin': 'pu', 'xr': 'pu', 'xs': 'pu'})
        self.calls.update({'gcall': True, 'jac0': True, 'fxcall': True, 'gycall': True, 'fcall': True, 'init1': True})
        self._inst_meta()
    ...
    def gcall(self, dae):
        ...
        dae.g += spmatrix(mul(self.u0, mul((dae.y[self.v])**2, div(1, self.xmu)) + mul(dae.x[self.ird], dae.y[self.v], self.xmu, div(1, self.x0))), self.v, [0]*self.n, (dae.m, 1), 'd')

    def fcall(self, dae):
        dae.f[self.theta_p] = mul(div(1, self.Tp), dae.x[self.theta_p] - mul(self.Kp, self.phi, dae.x[self.omega_m] - dae.y[self.omega_ref]))
        dae.anti_windup(self.theta_p, 0, 999999)
    ...
    def gycall(self, dae):
        ...
        dae.add_jac(Gy, mul(self.u0, mul(2, dae.y[self.v], div(1, self.xmu)) + mul(dae.x[self.ird], self.xmu, div(1, self.x0))), self.v, self.v)
