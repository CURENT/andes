...
class AVR1(ModelBase):
    ...
    def __init__(self, system, name):
        ...
        self._descr.update(
            {'busr': 'Regulated voltage idx', 'vrmax': 'Maximum regulator voltage', 'Kf': 'Stabilizer gain',
             ...
             'vrmin': 'minimum regulator voltage', 'Tr': 'Measurement time constant', 'Be': '2nd ceiling coefficient',
             'Tf': 'Stabilizer time constant', 'Ae': '1st ceiling coefficient',
             'Ke': 'Field circuit integral deviation', 'Te': 'Field circuit time constant'})
        self._data.update(
    ...
    def gcall(self, dae):
        ...
        dae.g += spmatrix(mul(self.u0, self.vf0 - dae.x[self.vfout]), self.vf, [0]*self.n, (dae.m, 1), 'd')

    def fcall(self, dae):
        dae.f[self.vm] = mul(div(1, self.Tr), dae.y[self.v] - dae.x[self.vm])
        vr1 = mul(self.Ka, dae.y[self.vref] - dae.x[self.vm] - dae.x[self.vr2] - mul(self.KfTf, dae.x[self.vfout]))
        dae.f[self.vr1] = div(vr1 - dae.x[self.vr1], self.Ta)
        dae.anti_windup(self.vr1, self.vrmin, self.vrmax)
        dae.f[self.vr2] = mul(div(1, self.Tf), -dae.x[self.vr2] - mul(self.KfTf, dae.x[self.vfout]))
        dae.f[self.vfout] = mul(div(1, self.Te), dae.x[self.vr1] - mul(dae.x[self.vfout], self.Ke + self.Se))
    ...
    def __init__(self, system, name):
        ...
        self._descr.update(
            {'K0': 'Regulator gain', 'vrmin': 'minimum regulator voltage', 'Ae': '1st ceiling coefficient',
             'Tr': 'Measurement time constant', 'syn': 'Generator id', 'T1': 'Regulator zero',
