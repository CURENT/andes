...
def pert(t, system):
    ...
    Arbitrary logic and calculations can be performed in this function to
    ``system``.
    ...
    To implement, add the following line to the scope where the event is
    triggered:
    ...
    line as it may cause significantly slow-down.

    The perturbation file can be supplied to the CLI using the ``--pert``
    argument or supplied to :py:func:`andes.run` using the ``pert`` keyword.
