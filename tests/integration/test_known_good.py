...
class TestKnownResults(unittest.TestCase):
    sets = (
        ("kundur/kundur_aw.json", "kundur_aw_10s.pkl", 10),
    ...
    def tnc(self, case_path, pkl_path, tf, pkl_prefix="pkl"):
        ...
        ss = compare_results(
            case_path, os.path.join(pkl_prefix, pkl_path), tf=tf, addfile=addfile
        )
...
def compare_results(case, pkl_name, addfile=None, tf=10):
    ...
    indices = np.hstack(
        (ss.GENROU.omega.a, ss.dae.n + ss.GENROU.tm.a, ss.dae.n + ss.GENROU.vf.a)
    )
    logger.info(case)
