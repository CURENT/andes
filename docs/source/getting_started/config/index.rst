...
ANDES uses a config file to store the various options for routines and models.
As discussed in :ref:`andes-misc`, the config file is at ``$HOME/.andes/andes.rc``.

Format
------

The ANDES config file uses the format provided by Python module
:py:mod:`configparser`. The syntax reads like the following:
...
In the above, ``System`` and ``PFlow`` are two sections, and ``freq = 60``, for
example, is a pair of option and value in the ``System`` section. Note the
single spaces before and after the equal sign.
